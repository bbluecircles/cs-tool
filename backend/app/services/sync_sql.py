...
# ---------------------------------------------------------------------------
# Targeted customer_code propagation (change-code safety net)
# ---------------------------------------------------------------------------
# Same idea as propagate_disable, for a customer's code rather than a user's
# disable flag. The user_details* tables denormalize customer_code, and this
# tool doesn't own the refresh (an external process does), so after a code
# change they'd keep the OLD code until that refresh lands. That's not
# cosmetic:
#   - mariadb_auth reads customer_code from secure.user_details_internal_2026
#     to decide admin access,
#   - every grant generator in this module selects users by customer_code.
# Both would look at the stale value. Renumbering in place costs one UPDATE
# per table and keeps them consistent immediately.
def propagate_customer_code(
    conn: Connection, *, old_code: int, new_code: int
) -> int:
    """Renumber customer_code from old to new across every user_details*
    table. Returns total rows updated. Per-table failures are logged but
    don't abort the rest — a customer with no rows in a given table just
    updates 0 rows there, which is fine."""
    total = 0
    for tbl in _USER_DETAILS_TABLES:
        try:
            r = conn.execute(
                text(
                    f"UPDATE {tbl} SET customer_code = :new "
            ...
            log.warning("propagate_customer_code failed on %s: %s", tbl, e)
    log.info(
        "propagate_customer_code(%s -> %s): %d rows", old_code, new_code, total
    )
    return total