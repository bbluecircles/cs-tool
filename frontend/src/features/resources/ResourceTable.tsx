...
interface ResourceTableProps {
  ...
  /** "Change code" row action (Customers tab) — renumbers customer_code. */
  onChangeCodeRow?: (row: Row) => void
...
export function ResourceTable({
  ...
  const onChangeCodeRowRef = useRef(onChangeCodeRow)
  onChangeCodeRowRef.current = onChangeCodeRow
  ...
  const columns = useMemo<ColumnDef<Row>[]>(() => {
      ...
      cell: ({ row }) => {
        ...
        return (
          <div className="flex items-center gap-1 min-w-[150px]">
            ...
            {config.allowChangeCode && onChangeCodeRowRef.current && (
              <button
                type="button"
                onClick={() => onChangeCodeRowRef.current?.(rowObj)}
                disabled={isSaving}
                className="text-xs px-2 py-0.5 rounded text-gray-700 hover:bg-gray-100 disabled:opacity-50"
                title="Change this customer's code (renumbers its users and datasets too)"
              >
                Change code
              </button>
            )}
    ...
    // refs inside the cells; the conditional Delete/Cancel/Change-code
