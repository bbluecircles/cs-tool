    ...
    <div ref={rootRef} className="relative">
      ...
      >
        {values.map((v) => (
          <span
            key={v}
            className="inline-flex items-center gap-1 rounded bg-secondary-100 text-secondary-700 px-1.5 py-0.5 text-xs"
          >
            {v}
            {!disabled && (
              <button
            ...
            {dbsQ.isLoading || excludeIsLoading ? (
              ...
              candidates.map((c) => (
                <button
                  key={c.database_name}
                  type="button"
                  className="block w-full text-left px-3 py-1.5 text-sm hover:bg-secondary-100/50"
                  onMouseDown={(e) => {
